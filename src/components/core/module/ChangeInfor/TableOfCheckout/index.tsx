...
function TableOfCheckoutForEdit({ isRefresh, search, setIsRefresh }: TProps) {
  ...
  const { scannedData, isFetching, refetch } = useAllCheckoutQuery(
    {
      search: search,
      limit: undefined,
    },
  ...
  const [editCheckout] = useEditCheckoutMutation();
  const handleEditById = async (data: any) => {
    try {
      const res = await editCheckout(data);
    } catch (error) {}
  };
  const [deleteCheckoutById] = useDeleteCheckoutByIdMutation();
  const handleDeleteById = async (id: string) => {
    try {
      const res = await deleteCheckoutById(id);
  ...
  const onFinish:FormProps<FieldType>["onFinish"] = async (values) => {
    ...
    try {
        const res = await editCheckout(updateData).unwrap();
        form.resetFields();
        closeModal();
        await refetch();
...
export default TableOfCheckoutForEdit;
