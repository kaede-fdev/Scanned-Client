...
import { useAllCheckinQuery, useAllCheckoutQuery } from "@/store/services/scan";
...
function TableCheckout({isRefresh}:TProps) {
    ...
    const { scannedCheckoutData, isFetching, refetch } = useAllCheckoutQuery(
      {
        limit: 10,
        search: search,
      },
      {
        selectFromResult: ({ data, isFetching }) => {
          return {
            scannedCheckoutData: data?.data?.scanneds ?? [],
            isFetching,
          };
        },
      }
    ...
    return (
      <Table
        columns={columns}
        dataSource={scannedCheckoutData}
        loading={isFetching}
